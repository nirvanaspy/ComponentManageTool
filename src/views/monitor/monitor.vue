<template>
  ...
  >
    <div
      class="selectedName"
      style="width:100%;height: 40px;line-height: 40px;color:#909399;"
    >
      <span v-if="this.selectedDeviceName" style="margin-right: 10px"
        >当前选中设备:{{ selectedDeviceName }}</span
      >
    ...
    <div
      style="width:33%;float:left;margin-top: 10px;border:1px solid #ebeef5;height: calc(100% - 110px);"
    >
      <el-table
        :data="deviceList"
      ...
      >
            ...
            >
              ...
              <el-table-column label="组件名" align="left" width="160">
                <template slot-scope="scope">
                  <span
                    class="link-type"
                    @dblclick="handleSelectComp(scope.row)"
                    >{{ scope.row.componentHistoryEntity.name }}</span
                  >
                </template>
              </el-table-column>
              <el-table-column label="路径" align="left">
                <template slot-scope="scope">
                  <span>{{
                    scope.row.componentHistoryEntity.relativePath
                  }}</span>
                </template>
              </el-table-column>
            </el-table>
        ...
        >
          <template slot-scope="scope">
            <span
              v-if="scope.row.deviceEntity !== null"
              class="link-type"
              @dblclick="getDetailByNode(scope.row, scope.$index)"
            >
              {{ scope.row.deviceEntity.name }}
              <svg-icon
                icon-class="scan_success"
                v-if="scope.row.scanSuccess"
              ></svg-icon>
            </span>
        ...
        <el-table-column align="left" label="IP" min-width="130">
          <template slot-scope="scope">
            <span v-if="scope.row.deviceEntity !== null" class="link-type">{{
        ...
        <el-table-column align="left" label="状态" width="100">
          <template slot-scope="scope">
            <span class="el-tag el-tag--danger" v-if="!scope.row.online"
              >离线</span
            >
    ...
    <div
      style="width:65%;height: calc(100% - 110px);float: right;margin-top: 10px;margin-bottom: 10px;border:1px solid #ebeef5"
    >
      <el-table
        :data="deviceDetail"
        v-loading="listLoading"
      ...
      >
        ...
        <el-table-column align="left" label="组件名" min-width="100">
          <template slot-scope="scope">
            <!--<span><svg-icon icon-class="组件"></svg-icon></span>-->
            <span class="link-type" :class="computedClass(scope.row)">{{
              scope.row.componentHistoryEntity.name
            }}</span>
        ...
        <el-table-column align="center" label="版本" width="80">
          <template slot-scope="scope">
            <span class="link-type" :class="computedClass(scope.row)">{{
              scope.row.componentHistoryEntity.version
            }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="扫描结果" min-width="130">
          <template slot-scope="scope">
            <span
              v-if="scope.row.correctFiles !== undefined"
              class="res-cont"
              @click="checkScanRes(scope.row, 'correct')"
            >
              <svg-icon icon-class="correct"></svg-icon>
              {{ scope.row.correctFiles.length }}
            </span>
            <span
              v-if="scope.row.modifyedFiles !== undefined"
              class="res-cont"
              @click="checkScanRes(scope.row, 'modifyed')"
            >
              <svg-icon icon-class="modifyed"></svg-icon>
              {{ scope.row.modifyedFiles.length }}
            </span>
            <span
              v-if="scope.row.missingFiles !== undefined"
              class="res-cont"
              @click="checkScanRes(scope.row, 'missing')"
            >
              <svg-icon icon-class="error"></svg-icon>
              {{ scope.row.missingFiles.length }}
            </span>
            <span
              v-if="scope.row.unknownFiles !== undefined"
              class="res-cont"
              @click="checkScanRes(scope.row, 'unknown')"
            >
              <svg-icon icon-class="unknown"></svg-icon>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          v-if="dialogStatus == 'scanByDevice'"
          type="primary"
          @click="scanQuickByDevice"
        >
          {{ $t("table.confirm") }}
        </el-button>
        <el-button v-else type="primary" @click="scanQuickByComp">{{
          $t("table.confirm")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    >
      ...
      <el-table
        class="searchResTable"
        :data="computedResList()"
        stripe
        fit
        width="100%"
      ...
      <el-tabs v-model="activeResPane" v-else>
        ...
        <el-tab-pane label="已修改" name="modifyed">
          <el-table
            :data="resDataList.modifyedFiles"
            fit
            width="100%"
            :show-header="false"
          >
  ...
  methods: {
        ...
        if (evt.target.result) {
          getResultByOrder(evt.target.result.orderId).then(res => {
            ...
            for (let j = 0; j < that.deviceDetail.length; j++) {
              that.expands.push(
                that.deviceDetail[j].deploymentDesignDetailEntity
                  .componentHistoryEntity.id
              );
              that.deviceDetail[j].componentHistoryEntity =
                that.deviceDetail[
                  j
                ].deploymentDesignDetailEntity.componentHistoryEntity;
              ...
              if (that.deviceList.length > 0) {
                for (let j = 0; j < that.deviceList.length; j++) {
                  if (that.deviceList[j].deviceEntity) {
                    if (
                      value.hostAddress ===
                      that.deviceList[j].deviceEntity.hostAddress
                    ) {
                      //查找在线设备
    ...
    handleSelectComp(row) {
      ...
      this.selectedDeviceName =
        row.deploymentDesignNodeEntity.deviceEntity.name;
      ...
      for (let i = 0; i < this.deviceList.length; i++) {
        if (this.selectedDeviceId == this.deviceList[i].id) {
          if (this.deviceList[i].online === true) {
            ...
            scanNode(this.selectedDeviceId)
              .then(res => {
                ...
                for (let j = 0; j < this.deviceDetail.length; j++) {
                  this.expands.push(
                    this.deviceDetail[j].deploymentDesignDetailEntity
                      .componentHistoryEntity.id
                  );
                  this.deviceDetail[
                    j
                  ].componentHistoryEntity = this.deviceDetail[
                    j
                  ].deploymentDesignDetailEntity.componentHistoryEntity;
        ...
        if (this.selectedDeviceId == this.deviceList[i].id) {
          if (this.deviceList[i].online === true) {
            ...
            scanNodeDetail(this.selectedCompId).then(res => {
              let uniqueId =
                this.deployPlanId + this.selectedDeviceId + this.selectedCompId;
      ...
      for (let i = 0; i < this.deviceList.length; i++) {
        if (this.selectedDeviceId == this.deviceList[i].id) {
          if (this.deviceList[i].online === true) {
            ...
            this.scanLoading = true;
            scanQuickNode(this.selectedDeviceId, this.type1)
              .then(res => {
                this.scanLoading = false;
                this.deviceDetail = res.data.data;
                this.expands = [];
                for (let k = 0; k < this.deviceDetail.length; k++) {
                  this.expands.push(
                    this.deviceDetail[k].deploymentDesignDetailEntity
                      .componentHistoryEntity.id
                  );
                  this.deviceDetail[
                    k
      ...
      for (let i = 0; i < this.deviceList.length; i++) {
        if (this.selectedDeviceId == this.deviceList[i].id) {
          if (this.deviceList[i].online === true) {
            ...
            this.scanLoading = true;
            scanQuickNodeDetail(this.selectedCompId, this.type1)
              .then(res => {
                this.scanLoading = false;
                this.deviceDetail = [];
                ...
                this.expands.push(
                  res.data.data.deploymentDesignDetailEntity
                    .componentHistoryEntity.id
                );
      ...
      let P = new Promise((resolve, reject) => {
        ...
        scanNode(deviceId)
          .then(res => {
            ...
            this.addOrderByPromise(uniqueId, res.data.data[0].orderId)
              .then(() => {
                this.$notify({
                  title: "成功",
