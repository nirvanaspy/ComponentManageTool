<template>
  <div class="app-container calendar-list-container" id="components">
    ...
    >
      ...
      <!-- 描述 -->
      <el-table-column width="200px" :label="$t('table.compDesc')">
        <template slot-scope="scope">
          <el-popover
            placement="bottom"
            title="描述"
          ...
          >
            ...
            <div slot="reference" class="td-description">
              {{ scope.row.description }}
            </div>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column min-width="200px" label="修改时间" sortable prop="tag">
        <template slot-scope="scope">
    ...
    >
      <div slot="title">
        <svg-icon icon-class="history1"></svg-icon>组件历史版本详情
      </div>
      <el-form
      ...
      >
        ...
        >
          ...
          <!-- <el-form-item :label="$t('table.compPath')" prop="relativePath">
            <el-input
              v-model="temp.relativePath"
              placeholder="/test/，必须以斜杠开头，斜杠结尾"
              disabled="disabled"
            ></el-input>
          </el-form-item> -->
          ...
          <!-- <el-form-item label="开发者">
            <el-input v-model="temp.developer" disabled></el-input>
          </el-form-item> -->
        ...
        >
          ...
          <comFileManage
            :selectCompId="selectedId"
            :selectCompName="selectdName"
          ></comFileManage>
        </div>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    exportLink(row) {
      let id = row.id
      this.exportUrl =
        service.defaults.baseURL + "/componenthistorys/" + id + "/export/"
    ...
    uploadCom: function(file) {
      ...
      importComp(this.userData, formData)
        .then(() => {
          uploading.close()
          this.$notify({
            title: "成功",
    ...
    showHistory: function() {
      ...
      compListHistory(this.projectId, this.listQuery)
        .then(response => {
          this.isHistory = true
          this.list = response.data.data.content
    ...
    showNow: function() {
      this.listLoading = true
      this.hisBtnLoading = true
      compList(this.projectId, this.listQuery)
        .then(response => {
          this.list = response.data.data.content
          this.total = response.data.total
          this.listLoading = false
