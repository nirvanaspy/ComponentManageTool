  ...
  <div class="project-container">
    <el-header style="background: #2d3a4b;line-height: 60px;">
      ...
      <div class="right-menu">
        <span
          calss="userName"
          style="position: relative;top: -12px;color: #fff;"
          >{{ userName }}</span
        >
        <el-dropdown class="avatar-container right-menu-item" trigger="click">
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item>
              ...
              <span v-else @click="handleModifyPassword" style="display:block;"
                >修改密码</span
              >
            </el-dropdown-item>
    ...
    >
      <el-tab-pane label="所有项目">
        <span class="tab-name" slot="label" style="font-size: 16px;"
          >所有项目<i class="el-icon-document" style="padding-left: 10px;"></i
        ></span>
        <div class="project-list">
          ...
          <div v-for="item in listA" v-if="!item.deleted" class="project-item">
            <div class="project-star project-detail">
              <span class="star-container" @click="toggleStar(item)">
                <svg-icon
                  v-if="item.hasStar"
                  icon-class="star-light"
                ></svg-icon>
                <svg-icon v-else icon-class="star-dark"></svg-icon>
              </span>
            </div>
            <div class="project-info project-detail">
              <div class="info-detail info-name" @click="handleSelect(item)">
                {{ item.name }}
              </div>
              <div class="info-detail info-description">
                {{ item.description }}
              </div>
            </div>
          ...
          >
            ...
            <div class="project-info project-detail">
              <div class="info-detail new-info" @click="handleCreate($event)">
                创建新项目
              </div>
            </div>
      ...
      <el-tab-pane label="回收站">
        <span slot="label" class="tab-name" style="font-size: 16px;"
          >回收站<i class="el-icon-delete" style="padding-left: 10px;"></i
        ></span>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          :disabled="this.btnConfirm"
          type="primary"
          @click="modifyPassword"
          :loading="modPasLoading"
          >确 定
        </el-button>
      </div>
    </el-dialog>
  ...
  methods: {
    ...
    createData() {
      ...
      this.$refs["dataForm"].validate(valid => {
        ...
        if (valid) {
          ...
          createProject(proData, this.userId)
            .then(() => {
    ...
    modifyPassword() {
      this.$refs["modifyPassForm"].validate(valid => {
        if (valid) {
          ...
          updateUser(datapost, this.userId)
            .then(() => {
              this.modPasLoading = false;
    ...
    showHistory: function() {
      ...
      if (this.role == "admin") {
        ...
        projectList_userHis(this.userId, this.listQuery)
          .then(response => {
            this.isHistory = true;
            this.list = response.data.data.content;
            this.listLoading = false;
    ...
    showNow: function() {
      ...
      if (this.role == "admin") {
        ...
        // alert(this.userId)
        projectList_user(this.userId, this.listQuery)
          .then(response => {
            this.list = response.data.data.content;
            this.listLoading = false;
            this.hisBtnLoading = false;
