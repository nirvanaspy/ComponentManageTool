          ...
          addUser(datapost)
            .then(res => {
              ...
              if (res.status === 200) {
                ...
                // this.$router.replace('/login')  (2) 注册完成后登录
                ...
                this.$store
                  ...
                  .then(res => {
                    ...
                    createProject(proData, this.userId).then(res => {
                      if (res.status === 200) {
                        // console.log(res);
                        this.$notify({
                          title: "成功",
                        ...
                        // 保存默认项目: (组件库管理) 的id到vuex
                        this.setProjectId(res.data.data.id)

                        //(4)默认项目创建完成后:跳到默认项目首页  :登录之后要准确的跳入默认建立的项目首页
                        this.$store.commit("SET_PROJECTID", res.data.data.id)
                        this.$store.commit("SET_PROJECTNAME", data.name)
...
.login-container1 {
  .el-input {
    ...
    input {
      ...
      height: 47px;
      &:-webkit-autofill {
        -webkit-box-shadow: 0 0 0px 1000px #0742cb inset !important;
