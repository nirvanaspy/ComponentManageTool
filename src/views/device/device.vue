<template>
  <div class="app-container calendar-list-container">
    <div class="filter-container">
      <div
        v-show="isHistory"
        style="position: absolute;top: 88px;font-size: 12px;color: #ccc;"
      >
        设备回收站
      </div>
      <el-input
        style="width: 240px;"
        class="filter-item"
        :placeholder="$t('table.deviceName')"
        v-model="searchQuery"
      >
      </el-input>
    ...
    >
      ...
      <el-table-column width="80px" align="center" label="操作系统">
        <template slot-scope="scope">
          <span style="font-size: 28px"
            ><svg-icon :icon-class="computedOs(scope.row.ostype)"></svg-icon
          ></span>
        </template>
      </el-table-column>
      <el-table-column
      ...
      <el-table-column min-width="90px" align="center" label="CPU">
        <template slot-scope="scope">
          ...
          <span v-else
            >{{ Math.round((scope.row.cpuClock / 1000) * 100) / 100 }}GHz</span
          >
        </template>
      </el-table-column>
      ...
      <el-table-column min-width="100px" align="center" label="内存利用率">
        <template slot-scope="scope">
          <span v-if="!scope.row.online">--</span>
          <span v-else-if="scope.row.ifChangeColor < 70"
            >{{ computedRamSize(scope.row) }}%</span
          >
          <span
            v-else-if="
              scope.row.ifChangeColor >= 70 && scope.row.ifChangeColor < 85
            "
            style="color: #FF8C00;"
            >{{ computedRamSize(scope.row) }}%</span
          >
          <span
            v-else-if="scope.row.ifChangeColor >= 85"
            style="color: #FF0000;"
            >{{ computedRamSize(scope.row) }}%</span
          >
        </template>
      ...
      <el-table-column align="center" :label="$t('table.deviceState')">
        <template slot-scope="scope">
          ...
          <span class="el-tag el-tag--danger" v-if="scope.row.online == false"
            >离线</span
          >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="primary"
            size="mini"
            v-if="scope.row.online && !scope.row.virtual"
            @click="handleProcess(scope.row)"
            >{{ $t("table.deviceProcess") }}</el-button
          >
          <el-button
            size="mini"
            type="success"
            v-if="scope.row.online && !scope.row.virtual"
            @click="handleDisk(scope.row)"
            >{{ $t("table.disk") }}</el-button
          >
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="primary"
            size="mini"
            v-if="scope.row.virtual"
            @click="handleReport(scope.row)"
            >{{ $t("table.report") }}</el-button
          >
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-dropdown
            trigger="click"
            v-if="!scope.row.virtual && scope.row.deleted"
          >
            <span class="el-dropdown-link">
              <el-button type="success" plain>更多操作</el-button>
            </span>
    ...
    >
      ...
      <lineMarker
        ...
        v-if="
          !isHistory &&
            dialogStatus === 'update' &&
            list[currentDeviceIndex].online == true
        "
    ...
    <el-dialog title="磁盘" :visible.sync="diskDialogVisible">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="diskDialogVisible = false">{{
          $t("table.close")
        }}</el-button>
      </div>
    </el-dialog>
    ...
    >
      ...
      >
        <el-form-item label="部署路径" prop="reportPath">
          ...
          <el-tooltip placement="top">
            ...
            <el-input
              v-model="pathTemp.reportPath"
              @keyup.enter.native.prevent="reportDevice"
              placeholder="例如：D:/test"
            />
          </el-tooltip>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="reportDevice">{{
          $t("table.confirm")
        }}</el-button>
      </div>
    </el-dialog>
    <el-dialog
      :visible.sync="classifyDeviceDialog.visible"
      class="classifyDialog"
    >
      <classify-device
        ref="classifyDevice"
        v-if="classifyDeviceDialog.visible"
      ></classify-device>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      pathRules: {
        reportPath: [
          { required: true, message: "请填写部署路径", trigger: "blur" }
        ]
      },
  ...
  methods: {
            ...
            $.each(that.webResBody, function(key, value) {
              ...
              if (that.list.length > 0) {
                for (let j = 0; j < that.list.length; j++) {
                  if (that.list[j].virtual !== true) {
                    //虚拟设备不需要再赋值  或者在每次查之前把虚拟且离线的设备删除
                    if (value.hostAddress === that.list[j].hostAddress) {
                      //查找在线设备
                      ...
                      that.list[j].ifChangeColor =
                        ((value.ramTotalSize - value.ramFreeSize) /
                          value.ramTotalSize) *
                        100;
    ...
    updateData() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          ...
          updateDevice(this.deviceId, data)
            .then(res => {
    ...
    copyDevice1(row) {
      ...
      copyDevices(copyId, this.userData, copyData)
        .then(res => {
          this.$notify({
            title: "成功",
            message: "复制成功",
    ...
    reportDevice() {
      this.$refs["reportForm"].validate(valid => {
        if (valid) {
          ...
          reportDevices(this.proId, RpData)
            .then(res => {
              // this.reportData.name = ''
              // this.reportData.ip = ''
              // this.reportData.deployPath = ''
  ...
  computed: {
    listA: function() {
      ...
      return self.list.filter(function(item) {
        return (
          item.name.toLowerCase().indexOf(self.searchQuery.toLowerCase()) !== -1
        );
      });
    },
    listenProjectId() {
      ...
      return function(row) {
        if (row.ramTotalSize && row.ramFreeSize) {
          if (row.ramTotalSize == 0) {
            ...
            return (
              Math.round(
                ((row.ramTotalSize - row.ramFreeSize) / row.ramTotalSize) *
                  10000
              ) / 100
            );
          }
