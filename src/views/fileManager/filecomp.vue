<template>
  <div class="fileComp">
    <div
      ...
      style="height: 40px;border-bottom:1px solid #ebeef5"
    >
      <div style="float: left;padding-left: 10px;">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          <el-breadcrumb-item
            ...
            :key="index"
            ><span
              style="cursor: pointer;color:rgb(0, 171, 235);"
      ...
      <div
        style="float: right;color:rgb(0, 171, 235);cursor: pointer;padding-right: 20px;"
      >
    ...
    >
      <el-table-column label="文件名" min-width="200">
        <template slot-scope="scope">
          <span>
            <svg-icon
              :icon-class="classifyIcon(scope.row)"
      ...
      <el-table-column width="120" :label="$t('table.compSize')">
        <template slot-scope="scope">
          <span v-if="scope.row.folder !== true">{{
            computedSize(scope.row.fileEntity.size)
          }}</span>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="!hiddenClose" @click="uploadDialog = false"
          >关 闭</el-button
        >
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button v-if="!hiddenClose" @click="uploadFolderDialog = false"
          >关 闭</el-button
        >
...
export default {
  ...
  methods: {
    ...
    upload_Folder() {
      ...
      uploadFolder(this.componentId, formData)
        .then(() => {
          this.$refs.uploaderFolder.uploader.cancel();
    ...
    exportFile(row) {
      ...
      let url =
        service.defaults.baseURL + "/componentfiles/" + row.id + "/export";
      window.open(url);
    },
    handleMove(row) {
...
<style>
/* 火狐48及以下版本不支持  background-position-y 属性:  vue-simple-uploader上传文件列表:删除图标显示不正确,base64流在源码中找   */
